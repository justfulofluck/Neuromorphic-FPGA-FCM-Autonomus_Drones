...
\large Neuromorphic Computing and FPGA-Based Spiking Neural Networks for Autonomous Drone Control}
\author{}
\date{}
...
\textbf{Critically for this thesis}, Xu et al. \cite{Xu2026} demonstrate that ANN-to-SNN conversion performs poorly in \textbf{continuous control} tasks. Performance degradation stems not from instantaneous action errors but from the progressive divergence of induced state trajectories; action approximation errors exhibit positive temporal correlation that amplifies even small conversion errors. This establishes that for continuous control---precisely the domain of autonomous drone flight---\textbf{direct surrogate-gradient training is necessary} rather than ANN-to-SNN conversion.
...
\section{SNN Applications in Drone and Robot Control}
\label{sec:drone}
Neuromorphic computing holds particular promise for small autonomous drones, where energy and weight budgets are severe \cite{Paredes2024}. Several works have progressed toward SNN-based flight control:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Hardware platform}: Neuromorphic drone control has been demonstrated on Loihi and microcontrollers \cite{Paredes2024,Stroobants2025}, but the affordable, reconfigurable FPGA alternative has not been systematically applied to this problem.
\end{enumerate}

\subsection{Positioning of This Thesis}
This thesis addresses the identified gaps by proposing a \textbf{surrogate-gradient-trained, fixed-point (Q4.11) quantized, FPGA-deployed LIF control network} for autonomous drone control. The contributions are:
\begin{itemize}
    \item A three-layer LIF control network (6$\rightarrow$64$\rightarrow$32$\rightarrow$4) mapping 6-DoF optical-flow sensory inputs to four PWM actuator commands through rate-coded spike accumulation.
...
\section{Conclusion}
This literature review surveyed SNN foundations, training methods, neuromorphic hardware, FPGA implementations, and neuromorphic drone control. It established that while SNNs trained via surrogate gradients and deployed on neuromorphic chips demonstrate compelling energy efficiency for drone control, a clear gap exists in \textbf{FPGA-based SNN implementations for continuous control tasks}. This thesis positions itself precisely within this gap, contributing a fixed-point quantized, FPGA-deployed LIF control network for autonomous drones.
